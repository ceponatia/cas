import { 
  ...
  NamedEntity, 
  VADState, 
  SignificanceScorer as ISignificanceScorer 
} from '@cas/types';
 ...
 */
export class SignificanceScorer implements ISignificanceScorer {
  ...
  private significantEvents = {
    relationship: ['friend', 'enemy', 'love', 'hate', 'marry', 'divorce', 'meet', 'leave'],
  ...
  private scoreEmotionalIntensity(content: string): number {
    let intensity = 0;
  ...
  private detectEmotionalChanges(content: string, context: WorkingMemoryTurn[]): EmotionalChange[] {
    ...
    entities.filter(e => e.type === 'PERSON').forEach(entity => {
      const currentVAD = this.estimateVADFromContent(content, entity.text);
      
      // Try to find previous VAD state from context
      const previousVAD = this.findPreviousVAD(entity.text, context) || 
                         { valence: 0, arousal: 0, dominance: 0 }; // Neutral default
  ...
  private extractNamedEntities(content: string): NamedEntity[] {
    ...
    quotedStrings.forEach(quoted => {
      ...
      entities.push({
        ...
        confidence: 0.6,
        start_pos: startPos,
        end_pos: startPos + quoted.length
      });
    });
  ...
  private classifyEntity(entity: string, context: string): 'PERSON' | 'PLACE' | 'OBJECT' | 'CONCEPT' {
    const lowerEntity = entity.toLowerCase();
    const lowerContext = context.toLowerCase();

    // Check for person indicators
    const personIndicators = ['said', 'told', 'asked', 'replied', 'thinks', 'feels', 'went'];
    if (personIndicators.some(indicator => lowerContext.includes(`${lowerEntity} ${indicator}`) || 
                                          lowerContext.includes(`${indicator} ${lowerEntity}`))) {
      return 'PERSON';
    }
    ...
    if (placeIndicators.some(indicator => lowerContext.includes(`${indicator} ${lowerEntity}`) ||
                                         lowerContext.includes(`${lowerEntity} ${indicator}`))) {
      return 'PLACE';
    }
  ...
  private estimateVADFromContent(content: string): VADState {
    ...
    this.emotionalKeywords.high_valence.forEach(word => {
      if (lowerContent.includes(word)) valence += 0.3;
    });
    
    this.emotionalKeywords.low_valence.forEach(word => {
      if (lowerContent.includes(word)) valence -= 0.3;
    });

    this.emotionalKeywords.high_arousal.forEach(word => {
      if (lowerContent.includes(word)) arousal += 0.3;
    });

    this.emotionalKeywords.high_dominance.forEach(word => {
      if (lowerContent.includes(word)) dominance += 0.3;
    });
  ...
  private findPreviousVAD(entityName: string, context: WorkingMemoryTurn[]): VADState | null {
    ...
    for (let i = context.length - 1; i >= 0; i--) {
      ...
      if (turn.content.toLowerCase().includes(entityName.toLowerCase())) {
        return this.estimateVADFromContent(turn.content, entityName);
      }
    }
    return null;
  }
